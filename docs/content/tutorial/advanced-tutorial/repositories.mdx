...
Note that the name of the job in the `repository` must match the name we declared for it in its `job` (the default is the function name). If these names don't match, you'll see a helpful error message.
...
Loading repositories via the `-f` option is actually just a convenience function. The underlying abstraction is the "workspace", which determines the set of repositories available to Dagit.

Dagster allows you to configure your workspace in a `workspace.yaml` file, which keeps you from having to type the same flags repeatedly to dagit.
